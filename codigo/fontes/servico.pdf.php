...
$pdf = new PDF('P'); // relat�rio em orienta��o "Paisagem"
$pdf->setHeader("");
$pdf->Open();

PDFServico($pdf, $gdb, $dbempresa );

$dateServico = date('Ymdis');

$pdf->Output('Servico'.$dateServico.'_n_'.$idser.'.pdf', 'D');

function PDFServico($pdf, $db, $dbempresa) {
   $xlinha =0;
   corpo($pdf, $db, $dbempresa );
}

function corpo($pdf, $db, $dbempresa ) {
   ...
   $pdf->SetXY(45, 2);
   $pdf->Cell(135, 3, $dbempresa->gs['NM_EMPR'][0],0,0,'C' );
   ...
   $pdf->SetXY(45, 25);
   $pdf->Cell(135, 3, 'Relatorio de Atendimento Tecnico',0,0,'C' );
   ...
   // AREA DO CLIENTE
   $dbempresa->buscar_cliente( $idcli );
   $cleinte = $dbempresa->gs['NOME'][0];
    
   $y = 35; 	
   $pdf->SetXY(2, $y);
   $pdf->Cell(185, 3, 'Cliente : '.$idcli.' - '.$dbempresa->gs['NOME'][0] );
   
   $pdf->SetXY(125, $y);
   $pdf->Cell(80, 3, 'Email : '.$dbempresa->gs['EMAIL'][0]);

   $pdf->SetXY(2, $y + 5);
   $pdf->Cell(185, 3, 'Endereço : '.$dbempresa->gs['ENDERECO'][0].'    Complemento: '.$dbempresa->gs['COMPLEMENTO'][0]  );
   
   $pdf->SetXY(2, $y + 10);
   $pdf->Cell(80, 3, 'Bairro : '.$dbempresa->gs['BAIRRO'][0]);
   
   $pdf->SetXY(100, $y + 10);
   $pdf->Cell(80, 3, 'Municipio : '.$dbempresa->gs['MUNICIPIO'][0]);

   $pdf->SetXY(2, $y + 15);
   $pdf->Cell(100, 3, 'Telefones / Celular : '.$dbempresa->gs['TELEFONE'][0].'    '.$dbempresa->gs['CELULAR'][0].'    '.$dbempresa->gs['FAX'][0]  );

   $pdf->SetXY(105, $y + 15);
   $pdf->Cell(90, 3, 'Contato : '.$db->gs['NOME'][0] );
   

   // AREA DO PRODUTO
   $dbempresa->buscar_produto_cliente($cd_eqpm, $idser);
   $y = 55;
   ...
   $pdf->SetXY(132, $y + 10);
   $pdf->Cell(70, 3, 'Situacao do Equipamento');

   $y = $y + 14;
   
   $dbempresa->buscar_equipamento($idser); 
   if( $dbempresa->linhas>0 ){
	   foreach($dbempresa->gs['DS_EQPM'] as $key => $value ) {
		   ...
		   $pdf->SetXY(54, $y + 1);
		   $pdf->MultiCell(18, 3, $dbempresa->gs['DT_EMSS'][$key],0,"J","L");		
		
		   $pdf->SetXY(74, $y + 1);
           $pdf->MultiCell(18, 3, $dbempresa->gs['DS_SERI'][$key],0,"J","L");		
		
		   $pdf->SetXY(94, $y + 1);
		   $pdf->MultiCell(18, 3, $dbempresa->gs['NM_REVN'][$key],0,"J",'L' );

		   $pdf->SetXY(114, $y + 1);
		   $pdf->MultiCell(18, 3,$dbempresa->gs['DS_MODL'][$key],0,"J","L");		
   ...
   $pdf->SetFont('Arial','B', 8);
   $pdf->SetXY(02, $y + 5);
   $pdf->Cell(30, 3,'Situacao do Servico : ',0,0,'L' ); 

   $pdf->SetFont('Arial','', 8);     
   $pdf->SetXY(32, $y + 5);
   $pdf->Cell(20, 3,iconv("UTF-8", "CP1252", $db->gs['EQUIPAMENTO'][0] ),0,0,'L' );   
   ...
   $pdf->SetFont('Arial', 'B', 10);
   $pdf->SetXY(45, $y + 5);
   $pdf->Cell(135, 3, 'Servicos Executados',0,0,'C' );
   $pdf->SetFont('Arial','B', 8);
	
   $pdf->SetXY(2,   $y + 12);
   $pdf->Rect( 2,   $y + 10, 20, 5, 'D');	
   $pdf->Rect( 22,  $y + 10, 80, 5, 'D');		
   $pdf->Rect( 102, $y + 10, 20, 5, 'D');		   
   $pdf->Rect( 122, $y + 10, 25, 5, 'D');		      
   $pdf->Rect( 147, $y + 10, 25, 5, 'D');		         
   $pdf->Rect( 172, $y + 10, 30, 5, 'D');		            
	
   $pdf->SetXY(2, $y + 11);
   $pdf->Cell(20, 3, 'Codigo',0,0,'C' );
   
   $pdf->SetXY(22, $y + 11);
   $pdf->Cell(80, 3, 'Descricao',0,0,'C' );
   ...
   for ($j = 0; $j < 96; $j=$j+8 ) {
	   $pdf->Rect( 2,  $y + $j, 20, 8, 'D');	
	   $pdf->Rect( 22, $y + $j, 80, 8, 'D');		
	   $pdf->Rect( 102,$y + $j, 20, 8, 'D');		   
	   $pdf->Rect( 122,$y + $j, 25, 8, 'D');		      
	   $pdf->Rect( 147,$y + $j, 25, 8, 'D');		         
	   $pdf->Rect( 172,$y + $j, 30, 8, 'D');		            
   }
   
   $j = $j + $y;
   */
   
   $db->buscar_itens( $idser );
   ...
   if( $db->linhas>0 ){
	   foreach($db->gs['IDPRO'] as $key => $value ) {
		   ...
		   $pdf->SetXY(104, $j + 2);
		   $pdf->Cell(20, 3, $db->gs['UNIDADE'][$key],0,0,'C' );
   ...
   $pdf->Rect( 172, $j, 30, 8, 'D');		               
   $pdf->SetXY(2, $j+3);   
   $pdf->Cell(170, 3, 'SubTotal ====>>',0,0,'R' );   
   

   $pdf->Rect( 2,   $j + 8, 170, 8, 'D');	   
   $pdf->Rect( 172, $j + 8, 30, 8, 'D');		               
   $pdf->SetXY(2, $j + 11);   
   ...
   $pdf->SetFont('Arial','', 8);	
   $pdf->SetXY(2, $y + 5 );   
   $pdf->MultiCell(95, 3,  $db->gs['SERVICO'][0],0,"J",'L');  
   
   $pdf->SetXY(102, $y  );      
   $pdf->Cell(95, 3, 'Observacoes :',0,0,'L' );  
   $pdf->SetXY(102, $y + 5 );   
   ...
   $pdf->SetXY(2, $y + 25 );   
   $pdf->Cell(195, 3, 'Declaro que efetuei o servico de acordo com o descrito acima. No periodo  das  ___/___ as ___/___  do dia _____/_____/_____  ',0,0,'L' );  
   
   $pdf->SetXY(2, $y + 35 );
   $pdf->Cell(100, 3, '____________________________________________________',0,0,'C' );  
   
   $pdf->SetXY(2, $y + 40 );
   $pdf->Cell(100, 3,$cleinte,0,0,'C' );  
   ...
   $pdf->SetXY(105, $y + 40 );   
   $pdf->Cell(95, 3, 'Tecnico : '.strtoupper( $tecnico ),0,0,'C' );  
