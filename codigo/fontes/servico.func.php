...
class servico extends gdb { 
   ...
   function insert(  $idser, 
      ...
      $this->parametro('dt_entr' ,'DIDATE',$dt_entr );
      $this->parametro('dt_sada' ,'DIDATE',$dt_sada );
   ...
   function update( 
      ...
      if ($this->open(' update knoll_servicos 
                      set idcli       = :idcli, 
												  dt_entr     = :dt_entr, 
												  dt_sada     = :dt_sada, 
												  val_pro     = :val_pro, 
   ...
   function buscar($tipo, $codigo='', $situacao = 0 ){
     ...
     $select  =  " select  idser, 
                	...
                	( select REPLACE(REPLACE(REPLACE(FORMAT( ( sum(VAL_TOT) ), 2),'.',';'),',','.'),';',',')    
                      from knoll_servicos_itens ss 
                    where ss.idser = s.idser and upper(ps)= 'P' )  as  VAL_PRO,

            		  ( select REPLACE(REPLACE(REPLACE(FORMAT( ( sum(VAL_TOT) ), 2),'.',';'),',','.'),';',',') 
                      from knoll_servicos_itens ss
                     where ss.idser = s.idser )  as  VAL_TOT, 
            			...
            			( select REPLACE(REPLACE(REPLACE(FORMAT( ( sum(VAL_TOT) ), 2),'.',';'),',','.'),';',',')   
                     from  knoll_servicos_itens ss 
                    where ss.idser = s.idser and upper(ps)= 'S' )  as VAL_SER,
            			...
            			( select REPLACE(REPLACE(REPLACE(FORMAT(VAL_DES, 2),'.',';'),',','.'),';',',')
                     from  knoll_servicos ss
                     where ss.idser = s.idser )  as  VAL_DES,
            						   hr_serv   
   ...
   /*      $select  =  " select  idser, 
                           ...
                           DATE_FORMAT(dt_entr, '%d/%m/%Y') as dt_entr, 
                           DATE_FORMAT(dt_sada, '%d/%m/%Y') as dt_sada,                
                  REPLACE(REPLACE(REPLACE(FORMAT((val_pro/100), 2), '.', '@'), ',', '.'), '@', ',') as val_pro, 
                  REPLACE(REPLACE(REPLACE(FORMAT((val_tot/100), 2), '.', '@'), ',', '.'), '@', ',') as val_tot, 
                           idpag, 
                           s.idfun, 
                           s.nome, 
                           idrp, 
                           ...
                           hr_entr, 
                           hr_sada, 
                  REPLACE(REPLACE(REPLACE(FORMAT((val_ser/100), 2), '.', '@'), ',', '.'), '@', ',') as val_ser,
                           idpro as cd_eqpm , 
                           cd_agnd, 
                           s.tipo, 
                           idpro, 
                           in_status,
                           f.nome as funcionario,
                  REPLACE(REPLACE(REPLACE(FORMAT((val_des/100), 2), '.', '@'), ',', '.'), '@', ',') as val_des,
                           hr_serv   
                        from knoll_servicos s 
	...
	if($this->linhas == 0 ){
	   ...
	   $this->open($select.$critica,0, $situacao );	
	}
   ...
   function buscar_cliente( $codigo='', 
     ...
     $this->open(" select  nome,
						   ...
						   cpf
					  from knoll_clientes 
					 where idcli = $codigo ",0,$situacao );	 
	   
   } 
   
   function buscar_produto_cliente( $codigo='', 
                                    $idser='',
                                    $situacao = 0 ){
     ...
     $this->open("  SELECT P.CD_EQPM,
			  	 ...
			  	 LEFT JOIN knoll_servicos_produtos S
					  ...
					  WHERE cd_eqpm = $codigo ",0, $situacao ); 
   }   

   function buscar_equipamento($idser, $situacao = 0 ){
     ...
     $this->open("  SELECT P.CD_EQPM,
						   ...
						   P.NU_NOTA
					  FROM knoll_clientes_produtos P
					  WHERE P.IDSER = $idser ",0,$situacao );	 
	   
   }      
   
   function buscar_itens_produto($codigo, $situacao = 0 ){
	   
     $this->open(" SELECT distinct idpro, 
                                   descricao, 
                                   unidade, 
                                   venda 
                              FROM knoll_produtos 
                             where IDPRO = $codigo ",0,$situacao );	 
...
*/	
   ...
   function procurar_servico_cliente($in_tipo,$ds_dado){
	  ...
	  elseif( $in_tipo == 5 ) $where = " and upper(c.bairro) like upper('%$ds_dado%') ";  
	  elseif( $in_tipo == 6 ) $where = " and upper(c.municipio) like upper('%$ds_dado%') ";  
	  elseif( $in_tipo == 7 ) $where = " and upper(p.ds_eqpm) like upper('%$ds_dado%') ";  	  
   ...
   function procurar_cliente($in_tipo,$ds_dado){
     ...
     if($in_tipo == 0){
	  ...
	  if($in_tipo == 1){
		$where = " where c.nome is not null and  upper(c.nome) like upper('%$ds_dado%') ";  
	  } 
	  ...
	  if( $in_tipo == 4 ) $where = " where c.nome is not null and  upper(c.bairro) like upper('%$ds_dado%') ";  
	  
	  if( $in_tipo == 5 || $in_tipo == 6 ) $where = " where c.nome is not null and upper(c.municipio) like upper('%$ds_dado%') ";  
