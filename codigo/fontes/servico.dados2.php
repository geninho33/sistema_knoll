...
if( $navegar == 'u'  || 
	...
	foreach($gdb->tabela as $value){
	  $campo =$campo.$gdb->gs[$value][0].'|';
		...
		$gdb->buscar_cliente( $codigo );
		foreach($gdb->tabela as $value){
		  $campo =$campo.$gdb->gs[$value][0].'|';
		}		
}else if($navegar == 'itens_produto'){		
		 $gdb->buscar_itens_produto( $codigo );
		 foreach($gdb->tabela as $value){
		   $campo =$campo.$gdb->gs[$value][0].'|';
		 }		
	      ...
	      // Excluindo os dados do equipamento do Cliente
		  $gdb->buscar_produto_cliente2($cd_eqpm, $idser);
		  foreach($gdb->tabela as $value){
			 $campo =$campo.$gdb->gs[$value][0].'|';
...
function tabela_itens( $codigo, $gdb,$in_status ){	
  ...
  if( $gdb->linhas>0 ){ ?>
	...
	<? foreach( $gdb->gs['IDPRO'] as $key=>$value ){ ?>
         <tr>
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['IDPRO'][$key]; ?></font></td>
           <td align="center"class="table_interna"><font class="edit"><? print $gdb->gs['PS'][$key]; ?></font></td>
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['DESCRICAO'][$key]; ?></font></td>     
           <td align="center"class="table_interna"><font class="edit"><? print $gdb->gs['UNIDADE'][$key]; ?></font></td>          
           <td align="right"class="table_interna"><font class="edit"><? print $gdb->gs['QTDE'][$key]; ?></font></td>               
           <td align="right"class="table_interna"><font class="edit"><? print $gdb->gs['VAL_UNI'][$key]; ?></font></td>                    
           <td align="right"class="table_interna"><font class="edit"><? print $gdb->gs['VAL_TOT'][$key]; ?></font></td> 
           <td align="center"class="table_interna">
        <? if($in_status !='Encerrado' ){ ?>
              <input type="button" value="Excluir" onclick="buscar_itens('E',<? print $gdb->gs['IDPRO'][$key]; ?>);" />
        <? } ?>
           </td>                         
        </tr>	   
    <? } ?>
...
function tabela_produtos( $gdb,$in_status ){		
  if( $gdb->linhas>0 ){ ?>
	...
	<? foreach( $gdb->gs['CD_EQPM'] as $key=>$value ){ ?>
         <tr>
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['DS_EQPM'][$key]; ?></font></td>
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['NU_NOTA'][$key]; ?></font></td>           
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['DS_MODL'][$key]; ?></font></td>
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['DT_EMSS'][$key]; ?></font></td>     
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['DS_SERI'][$key]; ?></font></td>          
           <td align="left"class="table_interna"><font class="edit"><? print $gdb->gs['NM_REVN'][$key]; ?></font></td>    
        <td align="justify"  width="330"class="table_interna"><font class="edit"><? print $gdb->gs['DEFEITO'][$key]; ?></font></td>        
           <td align="center"class="table_interna">
        <? if($in_status !='Encerrado' ){ ?>
              <input type="button" value="Alterar"  onclick="buscar_itens_produto('U',<? print $value; ?>);" />
              <input type="button" value="Excluir"  onclick="buscar_itens_produto('E',<? print $value; ?>);" />              
        <? } ?>
           </td>                         
        </tr>	   
    <? } ?>
